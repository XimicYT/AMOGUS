...
function drawCard(player) {
    ...
    let targetTier = (roll < 0.10 && !hasTier2) ? 2 : 1;
...
function startGame() {
    ...
    playerIds.forEach(id => {
        ...
        io.to(id).emit('game_start', {
            ...
            tasks: GAME_TASKS, walls: MAP_WALLS
        });
...
io.on('connection', (socket) => {
    socket.on('join_lobby', (playerName) => {
        ...
        players[socket.id] = {
            id: socket.id, name: playerName || `Player_${Math.floor(Math.random() * 1000)}`,
            isReady: false, role: null, x: 0, y: 0, inventory: [], lastCardPlayTime: 0
        };
    ...
    socket.on('play_card', (cardIndex) => {
        ...
        if (cardIndex >= 0 && cardIndex < p.inventory.length) {
            ...
            setTimeout(() => {
                if (activeGlobalEffects[cardId] && Date.now() >= activeGlobalEffects[cardId]) {
